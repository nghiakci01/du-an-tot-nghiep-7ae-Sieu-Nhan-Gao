...
<div x-data="chatBot({ mode: '{{ $chatbot_mode ?? 'rules' }}' })" x-init="initChat()"
    class="chatbot-widget chat-container">

    <!-- Chat Window -->
    <div x-show="isOpen" x-transition:enter="transition ease-out duration-400"
        x-transition:enter-start="opacity-0 translate-y-10 scale-95"
        x-transition:enter-end="opacity-100 translate-y-0 scale-100"
        x-transition:leave="transition ease-in duration-300"
        x-transition:leave-start="opacity-100 translate-y-0 scale-100"
        x-transition:leave-end="opacity-0 translate-y-10 scale-95" class="chat-window"
        :class="{ 'fullscreen': isFullscreen }" style="display: none;">

        <!-- ... (rest of the HTML remains similarly structured but with optimized Alpine bindings) ... -->
        ...
        <div class="chat-header">
            ...
            <div style="display: flex; gap: 8px;">
                <button @click="toggleFullscreen" class="chat-close" :title="isFullscreen ? 'Thu nhỏ' : 'Phóng to'">
                    <svg x-show="!isFullscreen" xmlns="http://www.w3.org/2000/svg" width="18" height="18" fill="none"
                    ...
                    <svg x-show="isFullscreen" xmlns="http://www.w3.org/2000/svg" width="18" height="18" fill="none"
                        ...
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                            d="M3 16h4v4m0-4L3 21m18-5h-4v4m0-4l4 5M3 8h4V4M7 8L3 3m18 5h-4V4m0 4l4-5" />
                    </svg>
                </button>
        ...
        <div class="chat-messages" id="chat-messages">
            <template x-for="(msg, index) in messages" :key="index">
                <div
                    :class="['message-wrapper', msg.isUser ? 'user' : (msg.sender_type === 'staff' ? 'staff' : 'bot')]">
                    <div class="message-box">
                        <!-- Text Message -->
                        <p x-show="!msg.products || msg.products.length === 0" x-text="msg.text"
                    ...
                    <p class="message-time" :style="{ textAlign: msg.isUser ? 'right' : 'left' }" x-text="msg.time">
                    </p>
                </div>
            </template>
        ...
        <div class="chat-input-area">
            <form @submit.prevent="sendMessage()" class="chat-form">
                <input type="text" x-model="newMessage" placeholder="Nhập tin nhắn..." class="chat-input"
                    :disabled="isLoading">

                <button type="submit" class="chat-send-btn" :disabled="!newMessage.trim() || isLoading">
    ...
    <!-- Toggle Button -->
    <button @click="toggle" x-show="!isOpen" x-transition:enter="transition ease-out duration-300"
        x-transition:enter-start="opacity-0 scale-50" x-transition:enter-end="opacity-100 scale-100"
        class="chat-toggle-btn">
        <!-- Chat Icon -->
        <svg xmlns="http://www.w3.org/2000/svg" width="28" height="28" style="color: white;" fill="none"
            viewBox="0 0 24 24" stroke="currentColor">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
