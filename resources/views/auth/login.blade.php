        ...
        <div class="row justify-content-center">
            <div class="col-lg-6 col-md-8">
                <div class="auth-container">
                    <!-- Tabs -->
                    <div class="auth-tabs">
                        <button type="button" class="auth-tab {{ !request()->has('tab') || request('tab') == 'login' ? 'active' : '' }}" data-tab="login">
                            Đăng nhập
                        </button>
                        <button type="button" class="auth-tab {{ request('tab') == 'register' ? 'active' : '' }}" data-tab="register">
                            Đăng ký
                        </button>
                    </div>
                    
                    <!-- Login Form -->
                    <form action="{{ route('login') }}" method="POST" class="auth-form {{ !request()->has('tab') || request('tab') == 'login' ? 'active' : '' }}" id="login-form">
                        @csrf
                        <div class="input-group">
                            <input type="email" name="email" class="auth-input" placeholder="Nhập email hoặc Tên đăng nhập" value="{{ old('email') }}" required>
                            @error('email')
