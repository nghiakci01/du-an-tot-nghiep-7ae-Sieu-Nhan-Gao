...
<div class="page-header py-3 bg-primary text-white mb-4 rounded shadow-sm">
    <div class="container-fluid">
        <div class="row align-items-center">
            <div class="col-8">
                <h5 class="text-white mb-0"><i class="feather icon-package me-2" aria-hidden="true"></i>Chi tiết #{{ $order->id }}</h5>
            </div>
...
<div class="container-fluid px-0">
                    ...
                    <div class="col-12">
                        <form action="{{ route('staff.orders.accept', $order->id) }}" method="POST">
                            ...
                            <button type="submit" class="btn btn-primary w-100 py-3 fw-bold">
                                <i class="feather icon-check me-2" aria-hidden="true"></i> NHẬN ĐI GIAO NGAY
                            </button>
                        </form>
                    </div>
                @elseif($order->status === \App\Models\Order::STATUS_SHIPPED)
                    <div class="col-12">
                        <form action="{{ route('staff.orders.complete', $order->id) }}" method="POST" enctype="multipart/form-data" class="bg-light p-3 rounded border">
                            @csrf
                            <div class="mb-3 text-start">
                                <label class="form-label fw-bold text-dark"><i class="feather icon-image me-1" aria-hidden="true"></i> Hình ảnh xác nhận (Bắt buộc)</label>
                                <input type="file" name="delivery_image" class="form-control" accept="image/*" capture="camera" autocomplete="off" required>
                                <div class="form-text small text-muted">Chụp ảnh người nhận hoặc kiện hàng tại nơi giao.</div>
                            </div>
                            <button type="submit" class="btn btn-success w-100 py-3 fw-bold shadow-sm" onclick="return confirm('Xác nhận đã giao hàng thành công và tải lên minh chứng hình ảnh?')">
                                <i class="feather icon-check-circle me-1" aria-hidden="true"></i> HOÀN THÀNH GIAO HÀNG
                            </button>
                        </form>
                    </div>
                    <div class="col-6">
                        <button type="button" class="btn btn-danger w-100 py-3 fw-bold shadow-sm" data-bs-toggle="modal" data-bs-target="#failModal">
                            <i class="feather icon-x-circle me-1" aria-hidden="true"></i> THẤT BẠI
                        </button>
                    </div>
                @endif
            </div>
    ...
    <div class="card border-0 shadow-sm mb-4">
        <div class="card-header bg-white border-bottom py-3">
            <h6 class="mb-0 fw-bold text-primary"><i class="feather icon-map me-2" aria-hidden="true"></i>Bản đồ đường đi</h6>
        </div>
        <div class="card-body p-0">
            <div id="map" style="height: 300px; width: 100%;"></div>
            <div class="p-3 bg-light fs-6 fw-bold text-center border-top">
                <i class="feather icon-map-pin text-danger me-2" aria-hidden="true"></i>
    ...
    <div class="card border-0 shadow-sm mb-4">
        <div class="card-header bg-white border-bottom py-3">
            <h6 class="mb-0 fw-bold text-success"><i class="feather icon-user me-2" aria-hidden="true"></i>Thông tin người nhận</h6>
        </div>
        <div class="card-body">
            <div class="mb-3">
            ...
            <div class="mb-3">
                ...
                <a href="tel:{{ $order->phone }}" class="fs-6 fw-bold text-primary text-decoration-none">
                    <i class="feather icon-phone-call me-1" aria-hidden="true"></i> {{ $order->phone ?? 'N/A' }}
                </a>
            </div>
            @if($order->note)
            <div class="p-3 bg-light rounded text-danger small">
                <i class="feather icon-file-text me-2" aria-hidden="true"></i> <strong>Ghi chú đơn:</strong> <em>"{{ $order->note }}"</em>
            </div>
            @endif
        </div>
    </div>
    ...
    <div class="card border-0 shadow-sm mb-4">
        <div class="card-header bg-white border-bottom py-3">
            <h6 class="mb-0 fw-bold text-info"><i class="feather icon-shopping-cart me-2" aria-hidden="true"></i>Chi tiết hàng hóa</h6>
        </div>
        <div class="card-body p-2">
            @foreach($order->items as $item)
            <div class="d-flex align-items-center p-2 mb-2 border-bottom last-child-no-border">
                <img src="{{ $item->product->image_url ?? asset('assets/images/default-product.png') }}" class="rounded me-3 border" width="45" height="45" style="object-fit:cover;" alt="{{ $item->product->name }}">
        ...
        <div class="modal-content border-0 shadow">
            <form action="{{ route('staff.orders.fail', $order->id) }}" method="POST">
                ...
                <div class="modal-header bg-danger text-white py-3">
                    <h5 class="modal-title text-white" id="failModalLabel">Lý do giao thất bại</h5>
                    <button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <div class="modal-body py-4">
                    <label for="delivery_note" class="form-label d-none">Lý do giao thất bại</label>
                    <textarea name="delivery_note" id="delivery_note" class="form-control rounded-3" rows="4" placeholder="VD: Khách hàng hẹn lại, không liên lạc được…" required></textarea>
                </div>
