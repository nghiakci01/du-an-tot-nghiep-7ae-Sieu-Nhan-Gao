(function ($) {
    ...
    $("#slider-range").slider({
        ...
        slide: function (event, ui) {
            // Update summary text
            $("#price_min_text").text(formatCurrency(ui.values[0]));
            $("#price_max_text").text(formatCurrency(ui.values[1]));

            // Update input boxes with commas
            $("#min_price_input").val(formatNumber(ui.values[0]));
            $("#max_price_input").val(formatNumber(ui.values[1]));
    ...
    // Initial values set
    $("#price_min_text").text(formatCurrency($("#slider-range").slider("values", 0)));
    $("#price_max_text").text(formatCurrency($("#slider-range").slider("values", 1)));
    $("#min_price_input").val(formatNumber($("#slider-range").slider("values", 0)));
    $("#max_price_input").val(formatNumber($("#slider-range").slider("values", 1)));

    // Handle manual input changes
    function updateSliderFromInput() {
        var minVal = parseInt($("#min_price_input").val().replace(/\./g, '')) || 0;
        var maxVal = parseInt($("#max_price_input").val().replace(/\./g, '')) || maxPrice;
        ...
        // Update summary text
        $("#price_min_text").text(formatCurrency(minVal));
        $("#price_max_text").text(formatCurrency(maxVal));

        // Re-format inputs with commas
        $("#min_price_input").val(formatNumber(minVal));
        $("#max_price_input").val(formatNumber(maxVal));
    }
    ...
    // Formatting on keyup (optional, for real-time comma addition)
    $("#min_price_input, #max_price_input").on("keyup", function () {
